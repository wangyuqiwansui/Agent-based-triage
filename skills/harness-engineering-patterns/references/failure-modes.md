...
| `FAIL_0014` | Reasoning Under-Route / 推理欠路由 | `COG_REASONING__TOP_ROUTING` | A lightweight path cannot satisfy required evidence or validators, or releases a false or unsafe result. / 轻量路径无法满足必需证据或验证器，或放行错误、不安全结果。 | Fail release closed; preserve the failed basis; upgrade only when evidence is sufficient, otherwise complement evidence or escalate; audit outcomes. / 默认阻断放行并保留失败依据；仅在证据充分时升级推理，否则补证或升级；回接后验审计。 |
| `FAIL_0015` | Reasoning Over-Route / 推理过路由 | `COG_REASONING__TOP_ROUTING` | An expensive path adds no outcome-backed quality, risk reduction, or validation gain. / 高成本路径没有带来后验支撑的质量、风险降低或验证收益。 | Run outcome-backed counterfactual audits, version scene thresholds, and never de-escalate protected work from cost alone. / 执行后验支撑的反事实审计，版本化场景阈值，绝不只因成本对受保护任务降档。 |
| `FAIL_0016` | Route Oscillation / 路由振荡 | `COG_REASONING__TOP_ROUTING` | The same evidence and policy repeatedly produce upgrades and de-escalations without progress. / 相同证据与策略反复触发升降档且没有进展。 | Enforce signal-fingerprint determinism, hysteresis, a positive switch cap, no-progress rules, and terminal escalation. / 强制信号指纹确定性、滞回、正整数换路上限、无进展规则和终态升级。 |
