...
Cell / 交织点: collaboration-routing / 协作 x 路由
...
- 论文坐标 / Article Coordinate: Collaboration / 协作 x Routing / 路由 (Routing / 路由).
...
- 问题 / Problem: The article leaves this intersection unnamed; use it only if workflow evidence shows that Collaboration / 协作 needs Routing / 路由. / 论文未命名该交织点；仅当工作流证据表明 Collaboration / 协作 需要 Routing / 路由 时使用。
...
- 适配信号 / Fit Signals: 需要把任务送到正确角色、团队、评审者或负责人 / Work must go to the right role, team, reviewer, or owner.
...
Observability Metrics File / 可观测性指标文件: [collaboration-routing-observability.md](collaboration-routing-observability.md)
