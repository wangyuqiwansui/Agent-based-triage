...
- 论文依据 / Article Basis: 矩阵列名模式 / Matrix-listed pattern; source table maps Collaboration / 协作 x Hierarchy / 层级 in arXiv:2605.13850. / 矩阵列名模式 / Matrix-listed pattern；来源表将 Collaboration / 协作 x Hierarchy / 层级 映射到该单元。
- 问题 / Problem: The matrix lists this named pattern for the cell; use it when Use when work should be delegated from high-level goals to subteams or subagents. / 当工作应从高层目标委派到子团队或子 Agent 时使用。 Core fit signal: 协作需要按角色、权限、责任或任务层级组织 / Collaboration is organized by role, permission, responsibility, or task level. / 矩阵在该单元列出此命名模式；当 Use when work should be delegated from high-level goals to subteams or subagents. / 当工作应从高层目标委派到子团队或子 Agent 时使用。 时使用。核心适配信号：协作需要按角色、权限、责任或任务层级组织 / Collaboration is organized by role, permission, responsibility, or task level。
- 架构方案 / Architectural Solution: Use Hierarchical Delegation / 层级委派 to delegate or constrain work across levels with roll-up evidence / 跨层级委派或约束工作，并向上汇总证据 within the Collaboration / 协作 capability. / 在 Collaboration / 协作 能力内使用 Hierarchical Delegation / 层级委派，delegate or constrain work across levels with roll-up evidence / 跨层级委派或约束工作，并向上汇总证据。
