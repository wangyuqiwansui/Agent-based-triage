...
- 问题 / Problem: The matrix lists this named pattern for the cell; use it when governance requires coordinated evidence, traces, metrics, and review. Core fit signal: Governance coordinates policy, approval, evidence, tools, and traceability. / 矩阵在该单元列出此命名模式；当治理需要协调证据、追踪、指标和评审时使用。核心适配信号：治理需要协调策略、审批、证据、工具和追踪。
