...
- 问题 / Problem: The matrix lists this named pattern for the cell; use it when context should be revealed by a controller as the workflow needs it. Core fit signal: Multiple observation tools, states, and dependencies need coordination. / 矩阵在该单元列出此命名模式；当上下文应由控制器按工作流需要逐步披露时使用。核心适配信号：多个观察工具、状态和依赖需要统一协调。
