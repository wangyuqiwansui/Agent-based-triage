...
- 论文坐标 / Article Coordinate: Action / 行动 x Loop / 循环 (Loop / 循环).
...
- 问题 / Problem: The article leaves this intersection unnamed; use it only if workflow evidence shows that Action / 行动 needs Loop / 循环. / 论文未命名该交织点；仅当工作流证据表明 Action / 行动 需要 Loop / 循环 时使用。
...
- 适配信号 / Fit Signals: 行动需要根据结果反复修正直到通过检查 / Actions repeat and adjust based on results until checks pass.
...
Observability Metrics File / 可观测性指标文件: [action-loop-observability.md](action-loop-observability.md)
