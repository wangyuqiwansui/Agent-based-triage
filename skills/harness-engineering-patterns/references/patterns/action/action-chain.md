...
- 问题 / Problem: The matrix lists this named pattern for the cell; use it when Use when action is represented as a deterministic sequence of prompts or tool steps. / 当行动由确定性的提示或工具步骤序列表示时使用。 Core fit signal: 行动步骤必须按顺序执行，前置结果决定后续动作 / Actions must run in sequence and prior results determine later actions. / 矩阵在该单元列出此命名模式；当 Use when action is represented as a deterministic sequence of prompts or tool steps. / 当行动由确定性的提示或工具步骤序列表示时使用。 时使用。核心适配信号：行动步骤必须按顺序执行，前置结果决定后续动作 / Actions must run in sequence and prior results determine later actions。
