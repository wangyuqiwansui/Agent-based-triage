...
- 论文依据 / Article Basis: 矩阵列名模式 / Matrix-listed pattern; source table maps Reflection / 反思 x Loop / 循环 in arXiv:2605.13850. / 矩阵列名模式 / Matrix-listed pattern；来源表将 Reflection / 反思 x Loop / 循环 映射到该单元。
- 问题 / Problem: The matrix lists this named pattern for the cell; use it when Use when verification failure should drive repair until an external check passes. / 当验证失败应驱动修复直到外部检查通过时使用。 Core fit signal: 反思结果不断驱动下一轮修正直到达标 / Reflection repeatedly drives correction until criteria are met. / 矩阵在该单元列出此命名模式；当 Use when verification failure should drive repair until an external check passes. / 当验证失败应驱动修复直到外部检查通过时使用。 时使用。核心适配信号：反思结果不断驱动下一轮修正直到达标 / Reflection repeatedly drives correction until criteria are met。
- 架构方案 / Architectural Solution: Use Self-Heal Loop / 自愈循环 to repeat observe, decide, act, and check until an exit condition is met / 重复观察、决策、行动和检查，直到满足退出条件 within the Reflection / 反思 capability. / 在 Reflection / 反思 能力内使用 Self-Heal Loop / 自愈循环，repeat observe, decide, act, and check until an exit condition is met / 重复观察、决策、行动和检查，直到满足退出条件。
