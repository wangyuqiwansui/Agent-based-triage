...
- 问题 / Problem: The matrix lists this named pattern for the cell; use it when verification failure should drive repair until an external check passes. Core fit signal: Reflection repeatedly drives correction until criteria are met. / 矩阵在该单元列出此命名模式；当验证失败应驱动修复直到外部检查通过时使用。核心适配信号：反思结果不断驱动下一轮修正直到达标。
