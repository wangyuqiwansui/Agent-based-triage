...
- 问题 / Problem: The matrix lists this named pattern for the cell; use it when reflection routes work to packaged evaluators, repair skills, or review routines. Core fit signal: Evaluation decides rework, escalation, handoff, or release path. / 矩阵在该单元列出此命名模式；当反思需要路由到封装评估器、修复技能或评审例程时使用。核心适配信号：评估结果决定返工、升级、交接或发布路径。
