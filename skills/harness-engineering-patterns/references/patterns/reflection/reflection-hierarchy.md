...
- 问题 / Problem: The matrix lists this named pattern for the cell; use it when reflection needs replay across levels of past attempts or lessons. Core fit signal: Evaluation happens by task, module, version, or risk level. / 矩阵在该单元列出此命名模式；当反思需要跨多层历史尝试或经验回放时使用。核心适配信号：评估需要按任务、模块、版本或风险层级进行。
