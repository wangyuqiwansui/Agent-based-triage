...
- 问题 / Problem: The matrix lists this named pattern for the cell; use it when independent hypotheses or solution paths should be explored together. Core fit signal: Multiple hypotheses, designs, or causes can be reasoned about independently and compared. / 矩阵在该单元列出此命名模式；当多个独立假设或方案路径应同时探索时使用。核心适配信号：多个假设、方案或根因可以独立推理后比较。
