...
Standalone operation requires a normalizer, router, needed mode executors, atomic budget controller, validator gate, append-only event store, replayable state machine, and terminal-output builder. The deterministic route reference is [`reasoning_router.py`](../runtime/reasoning_router.py), the execution kernel is [`reasoning_runtime.py`](../runtime/reasoning_runtime.py), and producer-side Schema, hash, cross-binding, and evidence-gate checks are [`reasoning_artifacts.py`](../runtime/reasoning_artifacts.py). Use `EventStore` only for ephemeral runs. `JsonlEventStore` provides modest single-writer restart-safe replay and stores an immutable terminal result in a self-hashed sibling `.results.json` sidecar. Use [`SqliteEventStore`](../runtime/reasoning_event_sqlite_store.py) as the single-node multi-writer reference: it serializes outer writes, reloads the authoritative stream inside the transaction, atomically commits a contiguous suffix, preserves cross-instance idempotency, rejects stale causal parents, and stores one immutable terminal result per run; Schema v2 migrates v1 without rewriting events. Reopening either durable adapter restores a saved result and validates it against the replayed terminal state. A crash after terminal events but before result persistence is recoverable by replay and rebuild; once saved, divergent content fails closed. Horizontal or remote writers still require an equivalent network database adapter. Start governed execution through `ReasoningEngine.create_run_from_contract()` so the normative contract remains the sole authority for identity, route, budget, validators, stop limits, governance, and binding hash; the lower-level `create_run()` remains a compact kernel/test adapter. Record at least identities, initial route and reason, budget limits, reservations and use, per-step evidence/action/observation/decision, validator bindings and results, switches, escalation reason, stop reason, and the immutable terminal result binding. / 独立运行需要标准化器、路由器、所需模式执行器、原子预算控制器、验证闸门、追加式事件存储、可回放状态机和终态输出生成器。确定性路由参考见 [`reasoning_router.py`](../runtime/reasoning_router.py)，执行内核见 [`reasoning_runtime.py`](../runtime/reasoning_runtime.py)，生产端 Schema、哈希、跨绑定与证据门校验见 [`reasoning_artifacts.py`](../runtime/reasoning_artifacts.py)。`EventStore` 只用于临时运行。`JsonlEventStore` 用于轻量单写者且需要重启安全重放的场景，并把不可变终态结果保存到自哈希的同级 `.results.json` 伴随文件。单节点多写者参考使用 [`SqliteEventStore`](../runtime/reasoning_event_sqlite_store.py)：它串行化最外层写入，在事务内重载权威流，原子提交连续后缀，保持跨实例幂等，拒绝陈旧因果父事件，并为每个运行保存一个不可变终态结果；Schema v2 不重写事件即可迁移 v1。重新打开任一持久化适配器都会恢复已保存结果并根据重放终态校验。终态事件之后、结果持久化之前发生崩溃可通过重放与重建恢复；结果一旦保存，分歧内容默认阻断。水平或远程写者仍需等价网络数据库适配器。受治理执行必须通过 `ReasoningEngine.create_run_from_contract()` 启动，使规范契约成为标识、路由、预算、验证器、停止上限、治理与绑定哈希的唯一权威；底层 `create_run()` 仅保留为紧凑内核/测试适配器。至少记录标识、初始路由及原因、预算上限、预留与消耗、逐步证据/动作/观察/决定、验证绑定及结果、换路、升级原因、停止原因及不可变终态结果绑定。

For workflow-route history specifically, [`workflow_route_ledger.py`](../runtime/workflow_route_ledger.py) is the single-writer JSONL reference and [`workflow_route_sqlite_ledger.py`](../runtime/workflow_route_sqlite_ledger.py) is the single-node transactional multi-writer reference. The SQLite adapter serializes writers, validates the authoritative chain inside the transaction, atomically appends the record and advances the head, supports idempotent JSONL migration, and fails closed on unknown Schema versions or damaged projections. Distributed deployments still require an equivalent deployment-owned database adapter. / 对工作流路由历史，JSONL 路由账本是单写者参考，SQLite 路由账本是单节点事务型多写者参考。SQLite 适配器会串行化写者、在事务内校验权威链、原子追加记录并推进头部、支持幂等 JSONL 迁移，并在 Schema 版本未知或记录投影损坏时默认阻断；分布式部署仍需部署方提供等价数据库适配器。
...
For `execution_mode: parallel`, compile the explicit branch blueprint and sealed contract with the [Reasoning Parallel Factory / 推理并行工厂](reasoning-parallel-factory.md), then execute only through `ParallelPlanSession`. Use `start_session()` for new runs and `resume_session()` for durable event-backed runs. Atomically reserve the full branch wave before the first branch starts; coordinate external workers with [`ParallelPathScheduler`](../runtime/reasoning_parallel_scheduler.py), path-local fencing tokens, heartbeats, and the compiled deadline policy. For crash-safe external dispatch, use `ParallelDispatchCoordinator` with [`reasoning_parallel_outbox.py`](../runtime/reasoning_parallel_outbox.py) so lease acquisition plus enqueue and branch completion plus outbox completion share database transactions. Delivery is at least once; expiring delivery claims recover dispatch crashes, while side-effecting consumers still require durable idempotency. A lease TTL expiry revokes ownership but leaves its logical path and reservation open for a higher-token lease; only the global deadline closes open paths. External workers publish through the coordinator's `close_leased_branch()` so stale or reassigned holders cannot submit. Use [`reasoning_event_sqlite_store.py`](../runtime/reasoning_event_sqlite_store.py) plus the SQLite outbox for one node, or [`reasoning_event_postgres_store.py`](../runtime/reasoning_event_postgres_store.py) plus [`reasoning_parallel_postgres_outbox.py`](../runtime/reasoning_parallel_postgres_outbox.py) for horizontal writers; the PostgreSQL adapter uses per-run transaction advisory locks and `FOR UPDATE SKIP LOCKED`, with real-server tests gated by `HARNESS_POSTGRES_DSN`. Close every planned path with an explicit terminal and candidate evidence or elimination reason; compare completed candidates under one common criteria-and-veto contract; review the full path inventory; preserve minority findings and all non-selected elimination reasons; bind the selected public candidate to higher-version evidence revisions; then call `finalize_selected_candidate()` to record external validator outcomes, apply the evidence release gate, and persist the sealed terminal result. Derive branch metrics through `reasoning_parallel_projection.py`. The reference v1 accepts public branch results but does not execute tools or side effects; reasoning topology never grants action authority. / 对 `execution_mode: parallel`，使用[推理并行工厂](reasoning-parallel-factory.md)编译显式分支蓝图与密封契约，随后只通过 `ParallelPlanSession` 执行。新运行使用 `start_session()`，持久化事件支持的运行使用 `resume_session()`。首个分支启动前原子预留完整分支波次；使用 [`ParallelPathScheduler`](../runtime/reasoning_parallel_scheduler.py)、路径局部栅栏令牌、心跳和已编译截止策略协调外部工作者。崩溃安全外部分派使用 `ParallelDispatchCoordinator` 与 [`reasoning_parallel_outbox.py`](../runtime/reasoning_parallel_outbox.py)，让租约获取与入队、分支完成与发件箱完成分别共享数据库事务。交付为至少一次；可过期交付领取用于恢复分派崩溃，产生副作用的消费者仍需持久幂等。租约 TTL 到期只撤销所有权，逻辑路径及预算预留保持开放供更高令牌租约接管；只有全局截止才关闭开放路径。外部工作者通过协调器的 `close_leased_branch()` 发布，陈旧或已被替换的持有者不能提交。单节点使用 [`reasoning_event_sqlite_store.py`](../runtime/reasoning_event_sqlite_store.py) 与 SQLite 发件箱；水平写者使用 [`reasoning_event_postgres_store.py`](../runtime/reasoning_event_postgres_store.py) 与 [`reasoning_parallel_postgres_outbox.py`](../runtime/reasoning_parallel_postgres_outbox.py)；PostgreSQL 适配器使用按运行事务 advisory lock 与 `FOR UPDATE SKIP LOCKED`，真实服务器测试由 `HARNESS_POSTGRES_DSN` 门控。每条计划路径均以显式终态及候选证据或淘汰原因关闭；完成候选使用统一准则与否决契约比较；综合时查阅完整路径清单，保留少数结论及全部未选路径的淘汰原因；把被选公开候选绑定到更高版本证据修订；随后调用 `finalize_selected_candidate()` 记录外部验证结果、执行证据放行门并持久化封存终态结果。通过 `reasoning_parallel_projection.py` 派生分支指标。参考实现 v1 接收公开分支结果，但不执行工具或副作用；推理拓扑绝不授予行动权限。

Interactive operation uses [Workflow Observability Probes / 工作流可观测性探针](workflow-observability-probes.md). Emit `task_received`, `task_normalized`, `route_selected`, `contract_established`, guarded `state_transitioned`, `step_started`, `action_dispatched`, `action_observed`, `step_closed`, applicable evidence/candidate/iteration events, parallel `parallel_path_updated` lease/deadline events, `mode_switched`, `validation_started`, `validation_completed`, `feedback_updated`, and `run_ended`. Before collection, resolve versioned probes with complete `true | false | unknown` condition states; missing or unknown applicability fails closed. Before metric publication, retain calculation inputs and recompute the result under the registry-owned minimum sample and finalized watermark window. Handle probe feedback as data completion, continue, switch, stop, escalate, or block according to severity and authority. / 交互运行使用[工作流可观测性探针](workflow-observability-probes.md)。发送任务接收、任务标准化、路由选择、契约建立、受门控状态转换、步骤开始、动作分派、动作观察、步骤关闭、适用的证据/候选/迭代事件、并行 `parallel_path_updated` 租约/截止事件、模式切换、验证开始、验证完成、反馈更新和运行结束事件。采集前以完整的 `true | false | unknown` 条件状态解析版本化探针；条件缺失或未知时默认阻断。指标发布前保留计算输入，并在注册表拥有的最小样本与已封窗 watermark 下重算结果；按严重级别与权限将探针反馈处理为补数、继续、换路、停止、升级或阻断。
