...
| `PROBE_0024` Generator-Critic Artifact Version Lineage / 生成评审工件版本谱系 | Generator-Critic artifact creation and review start; exact revision/digest/record hash, parent binding, unreviewed status, invalidated receipts, and shared admission/change-authorization assurance. / 生成评审工件创建与评审开始；精确修订/摘要/记录哈希、父版本绑定、未评审状态、失效回执及共享准入/改变授权保证。 | Block stale review, noncontiguous or unchanged revision, inherited review status, unlinked superseding content, and missing shared assurance. / 阻断陈旧评审、不连续或未变化修订、继承评审状态、无谱系替代内容及共享保证缺失。 | Review-version match, re-review compliance, version escape. / 评审版本匹配、复审合规、版本逃逸。 |
| `PROBE_0025` Generator-Critic Evidence And Risk Coverage / 生成评审证据与风险覆盖 | Exact review record; full criteria, evidence snapshots, supported findings, unsupported opinions, score evidence, and risk set. / 精确评审记录；完整判据、证据快照、有据问题、无据意见、评分依据及风险集合。 | Block missing criteria, stale or diagnostic-only gating evidence, unsupported automatic gates, and uncovered material risk. / 阻断判据缺失、过期或仅诊断门控证据、无据自动门控及重大风险未覆盖。 | Evidenced-finding, opinion-retention, risk-evidence coverage, rubber-stamp escape. / 有据问题、意见留存、风险证据覆盖、橡皮图章逃逸。 |
| `PROBE_0026` Generator-Critic Decision And Receipt Separation / 生成评审裁决与回执分权 | Review, policy decision, receipt, and release; distinct actors, triggered rules, issue/opinion buckets, exact bindings, evidence set, and shared revalidation assurance. / 评审、策略裁决、回执与发布；分离角色、触发规则、问题/意见分桶、精确绑定、证据集合及共享复验保证。 | Block critic-owned release, authority collapse, non-reproducible decision, missing receipt/assurance, or mismatched acceptance. / 阻断评审器自有发布、权限坍缩、不可复现裁决、回执/保证缺失或接受不匹配。 | Receipt coverage and release-integrity findings. / 回执覆盖与发布完整性发现。 |
| `PROBE_0027` Generator-Critic Re-review And Version Escape / 生成评审显式复审与版本逃逸 | Artifact/review/decision/receipt sequence across revisions and release. / 跨修订与发布的工件/评审/裁决/回执序列。 | Block old-receipt replay, unre-reviewed revision release, more than two critique passes, digest mismatch, or continued work after budget. / 阻断旧回执重放、未复审修订发布、超过两次评审、摘要不匹配或预算后继续。 | Re-review compliance, version escape, pass-budget compliance. / 复审合规、版本逃逸、批次预算合规。 |
| `PROBE_0028` Skill Candidate Recurrence And Provenance / 技能候选复现与来源 | Candidate nomination and distillation; distinct run, environment, external outcome, contribution, reflection assurance, stable step, parameter, assumption, and boundary-evidence bindings. / 候选提名与蒸馏；不同运行、环境、外部结果、贡献、反思保证、稳定步骤、参数、假设和边界证据绑定。 | Block duplicate or insufficient recurrence, non-success sources, unrelated reflection nomination, or unsupported boundaries. / 阻断重复或不足复现、非成功来源、无关反思提名或无证据边界。 | Candidate recurrence-evidence rate. / 候选复现证据率。 |
| `PROBE_0029` Skill Package Boundary And Supply Chain / 技能包边界与供应链 | Exact `TRIAL` manifest; bilingual core, parameter origins, tool/permission/dependency inventory, resource trust, rollback, and provenance. / 精确 `TRIAL` 清单；双语核心、参数来源、工具/权限/依赖清单、资源信任、回滚与来源。 | Block incomplete package, inventory mismatch, untrusted write dependency, or missing rollback. / 阻断不完整技能包、清单不匹配、不可信写依赖或缺少回滚。 | Skill Package contract completeness. / 技能包契约完整率。 |
| `PROBE_0030` Independent Skill Verification And Credential Binding / 技能独立验证与凭证绑定 | Verification start/end, exact suite/environment/manifest, five dimensions, counterexamples, failure paths, regression/gaming state, issuer and credential lineage. / 验证起止、精确套件/环境/清单、五维、反例、失败路径、回归/投机状态、签发者与凭证谱系。 | Block self-validation, partial-pass promotion, validator gaming, early credential, or any exact-binding mismatch. / 阻断自验、部分通过晋升、验证器投机、过早凭证或任何精确绑定不匹配。 | Five-dimension pass, `TRIAL`→`VERIFIED`, credential exact-binding. / 五维通过、`TRIAL`→`VERIFIED`、凭证精确绑定。 |
| `PROBE_0031` Skill Qualification And Atomic Alias Release / 技能资格与原子别名发布 | Qualification and release transitions; exact manifest/credential, stage evidence, CAS revisions, external alias receipt. / 资格与发布转换；精确清单/凭证、阶段证据、CAS 修订与外部别名回执。 | Block stage skip, authority mismatch, stale or failed CAS, digest escape, and traffic with inactive qualification. / 阻断跳阶段、权限不匹配、陈旧或失败 CAS、摘要逃逸和非活跃资格流量。 | Alias-switch integrity and stale-credential findings. / 别名切换完整率与过时凭证发现。 |
| `PROBE_0032` Real Skill Reuse Outcome And Version Window / 技能真实复用结果与版本窗口 | Router decision, real run, exact manifest/credential, route window, external outcome, observation time, attribution state. / 路由决定、真实运行、精确清单/凭证、路由窗口、外部结果、观察时间与归因状态。 | Exclude synthetic, shadow, pending, unknown, stale-version, and untrusted-attribution records; hard-alert exact-window violations. / 排除合成、影子、待定、未知、陈旧版本和不可信归因记录；精确窗口违规硬告警。 | Real-reuse success and version-window integrity. / 真实复用成功率与版本窗口完整率。 |
| `PROBE_0033` Skill Freshness Withdrawal And Re-verification / 技能新鲜度撤权与复验 | Ordered credential suspension, `VERIFIED`→`TRIAL` demotion, re-verification start, superseding credential, revocation, retirement, archive, event hash chain. / 有序凭证暂停、`VERIFIED`→`TRIAL` 降级、复验开始、替代凭证、撤销、退役、归档与事件哈希链。 | Block re-verification before withdrawal, old-credential reuse, invalid replacement lineage, or history discontinuity. / 阻断未撤权先复验、旧凭证复用、无效替换谱系或历史不连续。 | Pre-withdrawal compliance and stale-credential use. / 复验前撤权合规率与过时凭证使用率。 |
...
The machine-readable metric definitions and reference calculations live in [`metric_registry.json`](../runtime/metric_registry.json) and [`reasoning_metrics.py`](../runtime/reasoning_metrics.py); deployable probe definitions live in [`probe_registry.json`](../runtime/probe_registry.json), and mode-to-probe coverage is authoritative in [`probe_dependency_matrix.json`](../runtime/probe_dependency_matrix.json). Before collection, call `resolve_required_probes` with execution mode, supporting topologies, and a complete `condition_states` map for every applicable conditional feature. For an admitted reflection, additionally call `resolve_reflection_required_probes`; its core profile is `PROBE_0016` through `PROBE_0021`, with `PROBE_0022` activated for attribution claims and `PROBE_0023` for learning promotion. Pass `generator_critic=True` for `reflection-chain` to add exact-version probes `PROBE_0024` through `PROBE_0027`; pass `skill_package=True` for `reflection-routing` to require `PROBE_0023` and lifecycle probes `PROBE_0028` through `PROBE_0033`. Each tri-state mode condition is `true`, `false`, or `unknown`; missing or `unknown` fails closed, while only `true` activates the conditional probe. Persist the resolved versioned bindings with the collector plan. Metric state must distinguish computed values, observed zero, missing, unknown, not applicable, and insufficient sample. / 机器可读指标定义和参考计算位于 [`metric_registry.json`](../runtime/metric_registry.json) 与 [`reasoning_metrics.py`](../runtime/reasoning_metrics.py)；可部署探针定义位于 [`probe_registry.json`](../runtime/probe_registry.json)，模式到探针覆盖以 [`probe_dependency_matrix.json`](../runtime/probe_dependency_matrix.json) 为准。采集前必须以执行模式、支撑拓扑和覆盖全部适用条件特征的 `condition_states` 映射调用 `resolve_required_probes`。对已准入反思，还必须调用 `resolve_reflection_required_probes`；其核心档案为 `PROBE_0016` 至 `PROBE_0021`，声明归因时启用 `PROBE_0022`，晋升学习时启用 `PROBE_0023`。`reflection-chain` 传入 `generator_critic=True`，增加精确版本探针 `PROBE_0024` 至 `PROBE_0027`；`reflection-routing` 传入 `skill_package=True`，强制 `PROBE_0023` 与生命周期探针 `PROBE_0028` 至 `PROBE_0033`。每个模式条件只能是 `true`、`false` 或 `unknown`；缺失或 `unknown` 默认阻断，只有 `true` 才激活条件探针。将解析后的版本化绑定与采集计划一同持久化。指标状态必须区分已计算值、观测零值、缺失、未知、不适用和样本不足。
...
The 84 registered formulas are the implemented MVP core, not the complete long-term metric catalog. The authoritative `coverage` declaration in `metric_registry.json` separates `implemented`, `planned`, and `gate_eligible` identifiers. Implemented metrics that are not `gate_eligible` remain publication-grade diagnostics only; planned metrics may be used as design candidates only. Neither class may drive a statistical alert, block, release, or protected transition until promoted to `gate_eligible` with source-backed threshold evidence. Direct per-instance integrity rules—such as validator gaming, accepted change without independent revalidation, exact-version escape, or re-verification before withdrawal—remain hard guards rather than aggregate metric thresholds. / 已注册的 84 项公式是已实现的 MVP 核心，而不是长期指标目录的全集。`metric_registry.json` 中权威的 `coverage` 声明将指标区分为 `implemented`、`planned` 与 `gate_eligible`。已实现但不在 `gate_eligible` 中的指标仍只可作为可发布诊断；规划中指标只能作为设计候选。两者在基于数据来源与阈值证据晋升为 `gate_eligible` 前，均不得驱动统计告警、阻断、放行或受保护转换。验证器投机、缺少独立复验却接受改变、精确版本逃逸或未撤权先复验等逐实例完整性规则仍是硬闸门，而不是聚合指标阈值。
...
generator_critic_review_version_match_rate = exact_version_review_records / review_records
generator_critic_revision_rereview_compliance_rate = rereviewed_revised_artifacts / revised_artifacts_entering_release
generator_critic_receipt_coverage_rate = accepted_versions_with_valid_receipt / accepted_versions_entering_release
generator_critic_version_escape_rate = released_version_escapes / released_reviewed_artifacts
generator_critic_evidenced_finding_rate = evidenced_findings / reported_findings_and_opinions
generator_critic_opinion_retention_rate = retained_non_gating_opinions / unsupported_opinions
generator_critic_risk_evidence_coverage_rate = risk_items_with_check_and_evidence / declared_material_risk_items
generator_critic_rubber_stamp_escape_rate = accepted_artifacts_with_downstream_covered_defect / accepted_artifacts_with_outcome_evidence
generator_critic_pass_budget_compliance_rate = sessions_within_critique_pass_budget / generator_critic_sessions
skill_candidate_recurrence_evidence_rate = candidates_meeting_recurrence_policy / nominated_skill_candidates
skill_package_contract_completeness_rate = complete_skill_package_contracts / registered_trial_packages
skill_package_five_dimension_verification_rate = packages_passing_all_five_dimensions / completed_skill_package_evaluations
trial_to_verified_rate = trial_versions_promoted_verified / trial_versions_with_completed_evaluation
credential_exact_binding_rate = credentials_with_exact_binding / issued_skill_credentials
alias_switch_integrity_rate = valid_atomic_alias_switches / production_alias_switch_attempts
skill_reuse_success_rate = successful_real_skill_reuses / real_skill_reuses_with_determined_outcome
version_window_integrity_rate = real_reuses_with_valid_version_window / real_skill_reuses
reverification_prewithdrawal_compliance_rate = reverifications_with_prior_withdrawal / skill_reverifications_started
stale_credential_use_rate = reuses_with_stale_or_inactive_credential / real_skill_reuses
```

`route_stability_rate` is not route accuracy and remains diagnostic without an owned threshold and promotion evidence. Only outcome-backed or independently audited labels may populate `outcome_route_accuracy`, `underroute_rate`, or `overroute_rate`. `outcome_linkage_coverage` is implemented as diagnostic completeness, but outcome-backed correctness metrics remain non-gating until that coverage meets an owned threshold. `candidate_completion_rate`, `branch_diversity`, and `branch_record_completeness` are implemented diagnostics; they do not become gates merely because the formula exists. `path_convergence_rate` remains diagnostic until an owner enforces a candidate-completion threshold and approves minimum samples, drift controls, and promotion evidence. The seven chain-factory metrics, three parallel-factory diagnostics, eight tool-dispatch diagnostics, nine shared-reflection diagnostics, nine Generator-Critic diagnostics, and ten Skill Package diagnostics remain non-gating until owned thresholds and promotion evidence are approved. A zero aggregate does not suppress per-instance emergency anomalies: execution without admission, write execution without a lease, frontier escape, duplicate confirmed side effects, reflection without admission, validator gaming, acceptance without independent revalidation, a blocking regression, stale-version review, unsupported gating, receipt mismatch, release-version escape, production qualification without an exact credential, alias-version mismatch, real-reuse window contamination, or re-verification before withdrawal are direct integrity violations. Budget utilization is a vector over tokens, latency, model calls, tool calls, paths, iterations, retries, and cost; never sum heterogeneous units. Identity, privacy/governance, and probe self-health are universal dependencies. / `route_stability_rate` 不是路由准确率；在缺少负责人阈值与晋升证据时仅作诊断。只有真实后验或独立审计标签才能进入 `outcome_route_accuracy`、`underroute_rate` 或 `overroute_rate`。`outcome_linkage_coverage` 已作为诊断完整度实现，但在其达到负责人阈值前，后验正确性指标仍不可门控。`candidate_completion_rate`、`branch_diversity` 与 `branch_record_completeness` 已实现为诊断指标；公式存在不等于可以门控。只有负责人强制候选完成阈值，并批准最小样本、漂移控制和晋升证据后，`path_convergence_rate` 才可超越诊断用途。七项链工厂指标、三项并行工厂诊断、八项工具调度诊断、九项共享反思诊断、九项生成评审诊断与十项技能包诊断在负责人阈值和晋升证据获批前仍不可门控。聚合指标为零也不能压制逐实例紧急异常：未经准入执行、写执行无租约、能力前沿逃逸、重复已确认副作用、未准入即反思、验证器投机、缺少独立复验却接受、阻断级回归、陈旧版本评审、无据门控、回执不匹配、发布版本逃逸、无精确凭证却生产授资格、别名版本不匹配、真实复用窗口污染或未撤权先复验，均属于直接完整性违规。预算利用率是令牌、延迟、模型调用、工具调用、路径、迭代、重试和成本的向量，禁止累加异构单位。身份、隐私治理和探针自健康是通用依赖。

### Hard alerts / 硬告警

Raise a critical alert without waiting for a statistical baseline when a high-risk run lacks a mandatory validator; failed validation enters `completed`; an irreversible action lacks permission or approval; a tool starts without a sealed allow decision; a write starts without a current state-evidence binding and durable execution lease; a selected tool escapes the sealed capability frontier; a result-unknown action is directly retried; the same idempotency identity has multiple confirmed side effects; a key factual conclusion has no evidence; execution continues after budget overrun; a hard constraint drifts without approval; a terminal run lacks a reason; identity linkage is irrecoverably broken; sensitive data is not redacted; private chain-of-thought is captured; a change is applied before reflection admission, baseline freeze, or authorization; an accepted reflection lacks exact-version independent revalidation; a validator is weakened or changed without independent approval and rebasing; a blocking regression continues or is accepted; attribution exceeds its evidence; a reflection ends without a closed round and stop reason; Generator-Critic review targets a stale artifact; an unsupported opinion or unevidenced score drives automatic rejection; a changed artifact inherits review state; critic, policy, and release authority collapse; a receipt or required evidence is stale; or the released digest differs from the receipted digest. / 出现以下情况时无需等待统计基线，直接产生严重告警：高风险运行缺少必选验证器；验证失败却进入完成态；不可逆动作缺少权限或审批；工具在缺少封存放行决定时启动；写动作在缺少当前状态证据绑定和持久执行租约时启动；所选工具逃逸封存能力前沿；结果未知动作被直接重试；同一幂等身份出现多个已确认副作用；关键事实结论无证据；预算越界后仍继续；硬约束未经批准发生漂移；终态缺少原因；标识链不可恢复；敏感数据未脱敏；采集私密思维过程；反思未准入、未冻结基线或未授权就应用改变；接受反思缺少改变后精确版本的独立复验；验证器未经独立审批与基线重建被削弱或修改；存在阻断级回归却继续或接受；归因强度超过证据；反思结束时轮次与停止原因未闭合；生成评审指向陈旧工件；无据意见或无据评分驱动自动拒绝；改变后工件继承评审状态；评审、策略与发布权限坍缩；回执或必需证据过期；或实际发布摘要与回执摘要不一致。

For Skill Package lifecycle, also raise a critical alert for `VERIFIED` without an exact active credential, stage skipping, a missing or inconsistent CAS alias receipt, a real reuse outside the exact manifest/credential window, reuse with a suspended, expired, revoked, or superseded credential, or re-verification started before credential suspension and `TRIAL` demotion. / 对技能包生命周期，以下情况也直接产生严重告警：没有精确活跃凭证却进入 `VERIFIED`、跳过发布阶段、CAS 别名回执缺失或不一致、真实复用位于精确清单或凭证窗口外、使用已暂停、过期、撤销或已被替代凭证复用，或在暂停凭证并降为 `TRIAL` 前开始复验。
